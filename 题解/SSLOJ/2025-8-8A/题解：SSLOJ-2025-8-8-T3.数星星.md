...
那么 $n$ 个数中共有至少 $jr$ 个数已经确定为 $1$，剩下的 $(n-jr)$ 个位置中放 $(k-jr)$ 个 $1$，剩余的 $(n-jr)-(k-jr)=n-k$ 个位置为 $0$，即有下式表示放法数量: ([隔板法](学习/数学/数学笔记/排列组合/组合数.md#隔板法)中的 $\binom{n+k-1}{n}$ )
...
又 $jr\leq k {\color{red}\Rightarrow} j\leq \frac k r{\color{red}\Rightarrow} j\leq \lfloor \frac k r\rfloor{\color{red}\Rightarrow} t=\lfloor \frac k r\rfloor$，有
